...
// testPopBestLevelHeapBehavior is a helper to test heap behavior
func testPopBestLevelHeapBehavior(t *testing.T, isBid bool, prices []float64, expectedOrder []float64) {
	t.Helper()
	ob := NewOrderBook("BTC-USD")
	...
	for i, expectedPrice := range expectedOrder {
		level := ob.PopBestLevel(isBid)
		if !level.Price.Equal(decimal.NewFromFloat(expectedPrice)) {
			t.Errorf("Expected pop %d to be %f, got %s", i+1, expectedPrice, level.Price)
		}
	}
...
func TestPopBestLevelMaxHeapBehavior(t *testing.T) {
	...
	expectedOrder := []float64{50300.0, 50200.0, 50100.0, 50000.0}
	testPopBestLevelHeapBehavior(t, true, prices, expectedOrder)
}

func TestPopBestLevelMinHeapBehavior(t *testing.T) {
	prices := []float64{51300.0, 51000.0, 51200.0, 51100.0}
	expectedOrder := []float64{51000.0, 51100.0, 51200.0, 51300.0}
	testPopBestLevelHeapBehavior(t, false, prices, expectedOrder)
}
